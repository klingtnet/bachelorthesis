...
Wie man in \Cref{fig:wadlstructure} erkennen kann, entspricht die Struktur einer \gls{WADL}-Datei einem Baum. Aus diesem Grund eignet sich eine Baumstruktur für das Datenmodell des Generators besonders gut. Um die Zielsprache (siehe \cref{sec:language_model}) im internen Datenmodell abbilden zu können, wird ein \gls{AST} als Datenstruktur gewählt.
...
Er ist das Endprodukt eines Parsingschrittes des Quelltextes, im Gegensatz zum \emph{konkreten Syntaxbaum} (auch \emph{Parse Tree}) enthält der \textsc{Ast} keine Formatierungsspezifische Syntax (bspw. Klammern). 
...
\begin{figure}[htb]
    ...
    %\resizebox{\textwidth}{!}{
        ...
        \end{tikzpicture}        
    %}
    \caption{Beispiel \textsc{Ast} für den rekursiven euklidischen Algorithmus}
    \label{fig:ast}   
\end{figure}
