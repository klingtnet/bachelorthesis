...
Die Hauptmerkmale von \textsc{Xsd} sind nach \cite[Kapitel 3.2][]{taxonomyXMLSchema} die folgenden:
\begin{compactitem}
...
Durch die optionale Angabe von \emph{Häufigkeitsindikatoren} \ding{204} kann festgelegt werden wie oft ein Element an der definierten Stelle vorkommen darf. Entfällt dies, entspricht der Wert von \emph{min-, maxOccurs} "1", das heißt, das Element darf genau einmal an dieser Stelle vorkommen.
...
\[  
    \underbrace{\text{xmlns}}_{\text{\textsc{Xml}-Namespace}}
    :
    \overbrace{\text{tns}}^{\text{Namensraumbezeichner}}
    \texttt{=}
    \underbrace{
        \texttt{"http://api.spreadshirt.net"}
    }_{\text{Konkreter Namensraum}}
\]
